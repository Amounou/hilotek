...
function QuoteRequests() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => (await supabase.from("quote_requests").select("*").order("created_at", { ascending: false })).data ?? [],
  });
  const upd = async (id: string, status: string) => {
    ...
    else { toast.success("Statut mis à jour"); qc.invalidateQueries({ queryKey: ["admin-quotes"] }); }
  };
  return (
    <div className="space-y-4">
      ...
      <Card className="overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>N°</TableHead><TableHead>Client</TableHead><TableHead>Service</TableHead>
